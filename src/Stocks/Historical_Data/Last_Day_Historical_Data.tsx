...
interface Last_Day_Historical_dataState {
    series: {
        name: string;
        data: string[]
    }[];
...
const Last_Day_Historical_data: React.FC = () => {
    const [state, setState] = useState<Last_Day_Historical_dataState>({
        series: [
            {
    ...
    const getdata = async () => {
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/fetch_historical_last_day_data/${symbol_id}`, {
                method: "GET",
                headers: {
            ...
            if (res.status === 200) {
                ...
                let max_value = Math.max(...data.last_dau_data.map(item => item.Close)) ;
                let min_value = Math.min(...data.last_dau_data.map(item => item.Close));

                // Get all dates and close values
                const all_data = data.last_dau_data.map(item => item.Close);
                const all_date = data.last_dau_data.map(item => item.Date);
    ...
    return (
        <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pt-7.5 pb-5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
            <Breadcrumb pageName="Last Day Stock Data" />
            ...
            <div>
                <div id="Last_Day_Historical_data" className="-ml-5">
                    <ReactApexChart
...
export default Last_Day_Historical_data;
